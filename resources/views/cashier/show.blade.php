                                                    ...
                                                    <div id="receipt">
                                                        <div id="selected-items">
                                                            <div class="d-flex justify-content-between mb-2">
                                                                ...
                                                                <p class="mb-0 plate" data-customer-id="{{ $pendingTransaction->customer->id }}">{{ $pendingTransaction->customer->plate_number }}</p>
                                                            </div>
                                                            @foreach ($pendingTransaction->pendingItems as $pending)
                                                                <div class="d-flex justify-content-between mb-2 item-row">
                                                                    <p class="items-name mb-0" data-item-id="{{ $pending->item->id }}" data-size-id="{{ $pending->size->id ?? '' }}">
                                                                        {{ $pending->item->items_name }} {{ $pending->size->size ?? '' }}
                                                                    </p>
                                                                    <div class="quantity-controls d-flex align-items-center">
                                                                        <button type="button" class="quantity-left-minus btn btn-danger btn-sm mx-1" data-type="minus">-</button>
                                                                        <input type="text" class="quantity-input form-control form-control-sm text-center" value="{{ $pending->quantity }}" min="1">
                                                                        <button type="button" class="quantity-right-plus btn btn-success btn-sm mx-1" data-type="plus">+</button>
                                                                    </div>
                                                                    <p class="items-price mb-0 text-right" data-item-price="{{$pending->harga_items}}">{{ formatRupiah($pending->harga_items) }}</p>
                                                                </div>
                                                            @endforeach
                                                        </div>
                                                        ...
                                                        <div class="d-flex justify-content-between">
                                                            ...
                                                            <p class="mb-2" id="coupons" data-coupons="{{ $totalDiscount }}">{{ formatRupiah($totalDiscount) }}</p>
                                                        </div>

                                                        <div id="change">
                                                            <!-- Additional logic for change can be added here -->
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="form-outline form-white mb-2">
                                                        <select class="form-select" id="coupon-select" name="diskon">
                                                            ...
                                                            @foreach($coupons as $coupon)
                                                                <option value="{{ $coupon->id }}" {{ $pendingTransaction->pendingItems->contains('coupon_id', $coupon->id) ? 'selected' : '' }}>
                                                                    {{ $coupon->name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
...
<script>
    ...
    $('.category-btn').click(function () {
        ...
        $.ajax({
            url: `/admin/cashier/show-pending-transaction/item/${categoryId}`,
            method: 'GET',
    ...
    if (couponId) {
    $.ajax({
        url: `/admin/cashier/show-pending-transaction/coupon/${couponId}`,
        method: 'GET',
